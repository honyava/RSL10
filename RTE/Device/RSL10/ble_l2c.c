    ...
    switch(msg_id)
    {
        case L2CC_LECB_DISCONNECT_IND:
        {
            ...
            for(uint16_t i = 0; i < l2c_env.connectionCount; i++)
            {
                ...
                {
                    /*
                     * Ensure that the array element shifting is within the
                     * bounds of l2c_env.connectionInfo.
                     * l2c_env.connectionCount must not exceed L2C_CONNECTION_MAX.
                     */
                    uint16_t num_shifted_elements = l2c_env.connectionCount - i - 1;
                    if (num_shifted_elements > L2C_CONNECTION_MAX - i - 1)
                    {
                        num_shifted_elements = L2C_CONNECTION_MAX - i - 1;
                    }

                    memmove(&(l2c_env.connectionInfo[i]),
                            &(l2c_env.connectionInfo[i + 1]),
                            num_shifted_elements * sizeof(L2C_ConnectionInfo_t));
